...
def train(nepochs, batch_size, noise_size, checkpoint):
    ...
    with tf.variable_scope('G'):
        ...
        G_sample = dcgan_generator_mnist(gen_data_ph, y=None, prob=dropout_prob_ph)

    with tf.variable_scope('D') as scope:
        data_ph = tf.placeholder(tf.float32, shape=[None, 28, 28], name='data_ph')   
        D_real_logits, D_real, D_feat_real = dcgan_discriminator_mnist(data_ph, y=None, prob=dropout_prob_ph)
    with tf.variable_scope('D', reuse=True):
        D_fake_logits, D_fake, D_feat_fake = dcgan_discriminator_mnist(G_sample, y=None, prob=dropout_prob_ph)
